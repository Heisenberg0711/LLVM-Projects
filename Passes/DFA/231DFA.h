...
class DataFlowAnalysis {
  ...
  public:
	...
	//Helper functions to reduce the clutter
	Info* getInfoFromEdge(Edge e) {
		return EdgeToInfo[e];
	}

	Instruction * getInstrFromIndex (unsigned idx) {
    ...
    void runWorklistAlgorithm(Function * func) {
      ...
      std::map<Instruction *, unsigned>::iterator it = InstrToIndex.begin();
      ...
      {
        //Get the instruction index and obtain incoming/out-going edges
        unsigned instruction_idx = worklist.front();
        worklist.pop_front();
        ...
        getIncomingEdges(instruction_idx, &Incoming);
        getOutgoingEdges(instruction_idx, &OutGoing);
        flowfunction(IndexToInstr[instruction_idx], Incoming, OutGoing, info);

        for (unsigned i = 0; i < OutGoing.size(); i++)
        {
          if (!Info::equals(EdgeToInfo[Edge(instruction_idx, OutGoing[i])], info[i]))
          {
            EdgeToInfo[Edge(instruction_idx, OutGoing[i])] = info[i];
            worklist.push_back(OutGoing[i]);
          }
        }
      }
